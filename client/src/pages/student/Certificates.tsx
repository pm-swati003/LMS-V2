    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {CERTIFICATES.map((cert) => (
          <Dialog key={cert.id}>
            <Card className="group overflow-hidden border-border/50 hover:border-primary/20 transition-all card-hover cursor-pointer">
              <DialogTrigger asChild>
                <div className="relative aspect-4/3 bg-muted overflow-hidden">
                   ...
                   <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10">
                     <div className="size-16 rounded-xl glass flex items-center justify-center mb-4 shadow-sm group-hover:scale-110 transition-transform">
                       <Award className="size-8 text-primary" />
                     </div>
                     <h3 className="text-lg font-semibold glass px-3 py-1 rounded-md">{cert.title}</h3>
                     <span className="text-sm font-medium mt-2 glass px-2 py-0.5 rounded-md">Issued: {cert.date}</span>
                   </div>
                </div>
              </DialogTrigger>
            ...
            <DialogContent className="max-w-3xl p-0 overflow-hidden bg-background">
              <div className="relative aspect-[1.414/1] bg-card text-foreground p-12 flex flex-col items-center justify-center text-center border-8 border-double border-slate-200 m-4">
                 ...
                 <h2 className="text-4xl font-semibold tracking-tight text-slate-900 mb-4">Certificate of Completion</h2>
                 <p className="text-slate-500 text-lg mb-8 uppercase tracking-widest">This is to certify that</p>
                 
                 <div className="text-4xl font-semibold italic text-primary mb-8 border-b-2 border-slate-200 pb-2 px-12">
                   Student Name
                 </div>
